translate_binary_to_text = {
    '01000001': 'A', '01000010': 'B', '01000011': 'C', '01000100': 'D',
    '01000101': 'E', '01000110': 'F', '01000111': 'G', '01001000': 'H',
    '01001001': 'I', '01001010': 'J', '01001011': 'K', '01001100': 'L',
    '01001101': 'M', '01001110': 'N', '01001111': 'O', '01010000': 'P',
    '01010001': 'Q', '01010010': 'R', '01010011': 'S', '01010100': 'T',
    '01010101': 'U', '01010110': 'V', '01010111': 'W', '01011000': 'X',
    '01011001': 'Y', '01011010': 'Z',
    '01100001': 'a', '01100010': 'b', '01100011': 'c', '01100100': 'd',
    '01100101': 'e', '01100110': 'f', '01100111': 'g', '01101000': 'h',
    '01101001': 'i', '01101010': 'j', '01101011': 'k', '01101100': 'l',
    '01101101': 'm', '01101110': 'n', '01101111': 'o', '01110000': 'p',
    '01110001': 'q', '01110010': 'r', '01110011': 's', '01110100': 't',
    '01110101': 'u', '01110110': 'v', '01110111': 'w', '01111000': 'x',
    '01111001': 'y', '01111010': 'z', "00000000": " ", "11100000": " ",
    "00100000": " ", "10000000": " ", "01100000": " ", "11000000": " ",
    "01000000": " ", "10100000": " "
}


translate_text_to_binary = {
    'A': '01000001', 'B': '01000010', 'C': '01000011', 'D': '01000100', 'E': '01000101',
    'F': '01000110', 'G': '01000111', 'H': '01001000', 'I': '01001001', 'J': '01001010',
    'K': '01001011', 'L': '01001100', 'M': '01001101', 'N': '01001110', 'O': '01001111',
    'P': '01010000', 'Q': '01010001', 'R': '01010010', 'S': '01010011', 'T': '01010100',
    'U': '01010101', 'V': '01010110', 'W': '01010111', 'X': '01011000', 'Y': '01011001',
    'Z': '01011010', 'a': '01100001', 'b': '01100010', 'c': '01100011', 'd': '01100100',
    'e': '01100101', 'f': '01100110', 'g': '01100111', 'h': '01101000', 'i': '01101001',
    'j': '01101010', 'k': '01101011', 'l': '01101100', 'm': '01101101', 'n': '01101110',
    'o': '01101111', 'p': '01110000', 'q': '01110001', 'r': '01110010', 's': '01110011',
    't': '01110100', 'u': '01110101', 'v': '01110110', 'w': '01110111', 'x': '01111000',
    'y': '01111001', 'z': '01111010', " ": "00000000"
}


print("if you want to translate from binary to text press: 1")
print("if you want to translate from text to binary press: 2")
binary_or_text = input("your choose: ")

if binary_or_text == "1":
    binary = input("THE BINARY: ")

    binary_chunks = []

    for i in range(0, len(binary), 8):
        binary_chunks.append(binary[i:i + 8])

    for chunk in binary_chunks:
        if chunk in translate_binary_to_text:
            print(translate_binary_to_text[chunk], end="")


elif binary_or_text == "2":

    text_chunks = []

    text = input("THE TEXT: ")
    for i in range(0, len(text)):
        text_chunks.append(text[i:i + 1])

    for chunk in text_chunks:
        if chunk in translate_text_to_binary:
            print(translate_text_to_binary[chunk], end="")
